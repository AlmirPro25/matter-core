    ...
    match type_tag[0] {
        ...
        0x03 => {
            ...
            let s = String::from_utf8(bytes).map_err(|e| {
                Error::new(ErrorKind::InvalidData, format!("Invalid UTF-8: {}", e))
            })?;
...
fn deserialize_event_handler<R: Read>(reader: &mut R) -> Result<(String, EventHandler)> {
    ...
    let event = String::from_utf8(event_bytes).map_err(|e| {
        Error::new(ErrorKind::InvalidData, format!("Invalid UTF-8: {}", e))
    })?;
...
fn deserialize_instruction<R: Read>(reader: &mut R) -> Result<Instruction> {
    ...
    match opcode[0] {
        ...
        0xA0 => {
            ...
            let mut bytes = [0u8; 4];
            reader.read_exact(&mut bytes)?;
            Ok(Instruction::NewStruct(type_name, u32::from_le_bytes(bytes) as usize))
        }
...
mod tests {
    ...
    fn test_round_trip() {
        ...
        assert_eq!(deserialized.main_instructions.len(), 9);
        assert!(matches!(
