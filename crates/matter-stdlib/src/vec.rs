    ...
    fn call(&mut self, method: &str, args: Vec<Value>) -> Result<Value, String> {
        match method {
            ...
            "with_capacity" => {
                ...
                if args.len() != 1 {
                    return Err(format!("Vec.with_capacity expects 1 argument, got {}", args.len()));
                }
                let capacity = args[0].as_int().map_err(|e| format!("Vec.with_capacity: {}", e))?;
            ...
            "get" => {
                ...
                if let Value::List(items) = &args[0] {
                    ...
                    if idx < 0 || idx >= items.len() as i64 {
                        return Err(format!("Vec.get: index {} out of bounds (len={})", idx, items.len()));
                    }
            ...
            "set" => {
                ...
                if let Value::List(items) = args[0].clone() {
                    ...
                    if idx < 0 || idx >= new_items.len() as i64 {
                        return Err(format!("Vec.set: index {} out of bounds (len={})", idx, new_items.len()));
                    }
            ...
            "is_empty" => {
                ...
                if args.len() != 1 {
                    return Err(format!("Vec.is_empty expects 1 argument, got {}", args.len()));
                }
                if let Value::List(items) = &args[0] {
            ...
            "contains" => {
                ...
                if args.len() != 2 {
                    return Err(format!("Vec.contains expects 2 arguments, got {}", args.len()));
                }
                if let Value::List(items) = &args[0] {
            ...
            "index_of" => {
                ...
                if args.len() != 2 {
                    return Err(format!("Vec.index_of expects 2 arguments, got {}", args.len()));
                }
                if let Value::List(items) = &args[0] {
            ...
            "insert" => {
                ...
                if args.len() != 3 {
                    return Err(format!("Vec.insert expects 3 arguments, got {}", args.len()));
                }
                if let Value::List(items) = args[0].clone() {
                    let idx = args[1].as_int().map_err(|e| format!("Vec.insert: {}", e))?;
                    let mut new_items = (*items).to_vec();
                    if idx < 0 || idx > new_items.len() as i64 {
                        return Err(format!("Vec.insert: index {} out of bounds (len={})", idx, new_items.len()));
                    }
            ...
            "remove" => {
                ...
                if args.len() != 2 {
                    return Err(format!("Vec.remove expects 2 arguments, got {}", args.len()));
                }
                if let Value::List(items) = args[0].clone() {
                    let idx = args[1].as_int().map_err(|e| format!("Vec.remove: {}", e))?;
                    let mut new_items = (*items).to_vec();
                    if idx < 0 || idx >= new_items.len() as i64 {
                        return Err(format!("Vec.remove: index {} out of bounds (len={})", idx, new_items.len()));
                    }
            ...
            "extend" => {
                ...
                if args.len() != 2 {
                    return Err(format!("Vec.extend expects 2 arguments, got {}", args.len()));
                }
            ...
            "slice" => {
                ...
                if let Value::List(items) = &args[0] {
                    ...
                    if start > items.len() || end > items.len() || start > end {
                        return Err(format!("Vec.slice: index out of bounds (start={}, end={}, len={})", 
                            start, end, items.len()));
                    }
            ...
            "reverse" => {
                ...
                if args.len() != 1 {
                    return Err(format!("Vec.reverse expects 1 argument, got {}", args.len()));
                }
                if let Value::List(items) = args[0].clone() {
                    let mut new_items = (*items).to_vec();
                    new_items.reverse();
...
mod tests {
    ...
    fn test_new_and_push() {
        ...
        // Push elements
        let vec = backend.call("push", vec![vec.clone(), Value::Int(10)]).unwrap();
        let vec = backend.call("push", vec![vec.clone(), Value::Int(20)]).unwrap();
        let vec = backend.call("push", vec![vec.clone(), Value::Int(30)]).unwrap();
        ...
        // Get elements
        let val = backend.call("get", vec![vec.clone(), Value::Int(0)]).unwrap();
        assert_eq!(val, Value::Int(10));
        
        let val = backend.call("get", vec![vec.clone(), Value::Int(2)]).unwrap();
    ...
    fn test_contains_and_index_of() {
        ...
        // Contains
        let result = backend.call("contains", vec![vec.clone(), Value::Int(20)]).unwrap();
        assert_eq!(result, Value::Bool(true));
        
        let result = backend.call("contains", vec![vec.clone(), Value::Int(99)]).unwrap();
        assert_eq!(result, Value::Bool(false));
        
        // Index of
        let result = backend.call("index_of", vec![vec.clone(), Value::Int(30)]).unwrap();
        assert_eq!(result, Value::Int(2));
        
        let result = backend.call("index_of", vec![vec.clone(), Value::Int(99)]).unwrap();
    ...
    fn test_insert_and_remove() {
        ...
        // Insert at index 1
        let vec = backend.call("insert", vec![vec, Value::Int(1), Value::Int(20)]).unwrap();
        
        // Should be [10, 20, 30]
        let val = backend.call("get", vec![vec.clone(), Value::Int(1)]).unwrap();
    ...
    fn test_slice() {
        ...
        // Slice [1..4)
        let result = backend.call("slice", vec![vec, Value::Int(1), Value::Int(4)]).unwrap();
