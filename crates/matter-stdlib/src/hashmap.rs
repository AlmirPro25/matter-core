    ...
    fn call(&mut self, method: &str, args: Vec<Value>) -> Result<Value, String> {
        match method {
            ...
            "insert" => {
                ...
                if args.len() != 3 {
                    return Err(format!(
                        "HashMap.insert expects 3 arguments, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = args[0].clone() {
                    let key = args[1]
                        .as_string()
                        .map_err(|e| format!("HashMap.insert: key must be string: {}", e))?;
            ...
            "get" => {
                ...
                if args.len() != 2 {
                    return Err(format!(
                        "HashMap.get expects 2 arguments, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
                    let key = args[1]
                        .as_string()
                        .map_err(|e| format!("HashMap.get: key must be string: {}", e))?;
                    map.get(&key)
                        .cloned()
            ...
            "get_or_default" => {
                ...
                if args.len() != 3 {
                    return Err(format!(
                        "HashMap.get_or_default expects 3 arguments, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
                    let key = args[1]
                        .as_string()
                        .map_err(|e| format!("HashMap.get_or_default: {}", e))?;
            ...
            "contains_key" => {
                ...
                if args.len() != 2 {
                    return Err(format!(
                        "HashMap.contains_key expects 2 arguments, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
                    let key = args[1]
                        .as_string()
                        .map_err(|e| format!("HashMap.contains_key: {}", e))?;
            ...
            "remove" => {
                ...
                if args.len() != 2 {
                    return Err(format!(
                        "HashMap.remove expects 2 arguments, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = args[0].clone() {
                    let key = args[1]
                        .as_string()
                        .map_err(|e| format!("HashMap.remove: {}", e))?;
                    let mut new_map = (*map).clone();
            ...
            "keys" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.keys expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
                    let keys: Vec<Value> =
                        map.keys().map(|k| Value::new_string(k.clone())).collect();
            ...
            "values" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.values expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
            ...
            "len" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.len expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
            ...
            "is_empty" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.is_empty expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
            ...
            "clear" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.clear expects 1 argument, got {}",
                        args.len()
                    ));
                }
                Ok(Value::new_map(StdHashMap::new()))
            ...
            "merge" => {
                ...
                if args.len() != 2 {
                    return Err(format!(
                        "HashMap.merge expects 2 arguments, got {}",
                        args.len()
                    ));
                }
            ...
            "from_pairs" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.from_pairs expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::List(pairs) = &args[0] {
                    ...
                    for pair in pairs.iter() {
                        if let Value::List(kv) = pair {
                            if kv.len() != 2 {
                                return Err(
                                    "HashMap.from_pairs: each pair must have exactly 2 elements"
                                        .to_string(),
                                );
                            }
                            let key = kv[0]
                                .as_string()
                                .map_err(|e| format!("HashMap.from_pairs: {}", e))?;
                            map.insert(key, kv[1].clone());
                        } else {
                            return Err(
                                "HashMap.from_pairs: each element must be a pair (list of 2)"
                                    .to_string(),
                            );
                        }
                    }
            ...
            "to_pairs" => {
                ...
                if args.len() != 1 {
                    return Err(format!(
                        "HashMap.to_pairs expects 1 argument, got {}",
                        args.len()
                    ));
                }
                if let Value::Map(map) = &args[0] {
                    let pairs: Vec<Value> = map
                        .iter()
                        .map(|(k, v)| {
                            Value::new_list(vec![Value::new_string(k.clone()), v.clone()])
                        })
                        .collect();
            ...
            "map_values" => {
                ...
                // Note: Needs function references
                Err(
                    "HashMap.map_values: not yet implemented (needs function references)"
                        .to_string(),
                )
            }
...
mod tests {
    ...
    fn test_new_and_insert() {
        ...
        let name = backend
            .call(
                "get",
                vec![map.clone(), Value::new_string("name".to_string())],
            )
            .unwrap();
        ...
        let version = backend
            .call(
                "get",
                vec![map.clone(), Value::new_string("version".to_string())],
            )
            .unwrap();
    ...
    fn test_contains_key() {
        ...
        let result = backend
            .call(
                "contains_key",
                vec![map.clone(), Value::new_string("key1".to_string())],
            )
            .unwrap();
        ...
        let result = backend
            .call(
                "contains_key",
                vec![map.clone(), Value::new_string("key2".to_string())],
            )
            .unwrap();
    ...
    fn test_remove() {
        ...
        // Remove key "a"
        let result = backend
            .call("remove", vec![map, Value::new_string("a".to_string())])
            .unwrap();
    ...
    fn test_merge() {
        ...
        let b_val = backend
            .call(
                "get",
                vec![merged.clone(), Value::new_string("b".to_string())],
            )
            .unwrap();
    ...
    fn test_from_pairs_to_pairs() {
        ...
        let pairs = Value::new_list(vec![
            Value::new_list(vec![Value::new_string("k1".to_string()), Value::Int(10)]),
            Value::new_list(vec![Value::new_string("k2".to_string()), Value::Int(20)]),
        ]);
