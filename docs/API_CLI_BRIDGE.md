...
`project-run-build-json` uses the same cacheable build contract and then executes the generated bytecode, returning the runtime output in the same JSON response.

`project-emit-build-json` builds the project to the same cacheable artifact shape and emits a named event from that bytecode, which lets event-driven apps expose one stable API call per event.
