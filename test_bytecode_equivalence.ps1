...
foreach ($file in $testFiles) {
    ...
    $sourceOutput = & .\target\release\matter-cli.exe run $file 2>&1 | Out-String
    ...
    & .\target\release\matter-cli.exe compile $file -o $bytecodeFile 2>&1 | Out-Null
    
    # 3. Executar bytecode
    Write-Host "  3. Executando bytecode..."
    $bytecodeOutput = & .\target\release\matter-cli.exe run-bytecode $bytecodeFile 2>&1 | Out-String
